...
type Entry struct{Digest string `json:"digest"`;FetchedAt time.Time `json:"fetched_at"`}
type Cache struct{Path string;Data map[string]Entry}
func Load(p string)(*Cache,error){
 c:=&Cache{Path:p,Data:map[string]Entry{}}
 b,err:=os.ReadFile(p); if err==nil{json.Unmarshal(b,&c.Data)}
 return c,nil
}
